...
public class CassandraServer implements Cassandra.Iface
{
                                                            ...
                                                            boolean reversed)
    {
        ...
        else
        {
            if (isCounterCF)
                ...
                return thriftifySuperColumns(metadata, cells, reversed);
        }
    }

    private List<ColumnOrSuperColumn> thriftifySuperColumns(CFMetaData metadata, Iterator<LegacyLayout.LegacyCell> cells, boolean reversed)
    {
        ArrayList<ColumnOrSuperColumn> thriftSuperColumns = new ArrayList<ColumnOrSuperColumn>();
    ...
    private PartitionFilter toInternalFilter(CFMetaData metadata, ColumnParent parent, SliceRange range)
    {
        ...
        try
        {
            ...
            if (metadata.isSuper())
            {
                ...
                if (column_path.column == null)
                {
                    ...
                    columns = ColumnsSelection.withoutSubselection(metadata.partitionColumns());
                }
                else
                {
                ...
                filter = new NamesPartitionFilter(columns,
                                                  FBUtilities.<Clustering>singleton(new SimpleClustering(column_path.super_column), metadata.comparator),
                                                  false);
            }
            else
            {
                LegacyLayout.LegacyCellName cellname = LegacyLayout.decodeCellName(metadata, column_path.super_column, column_path.column);
                filter = new NamesPartitionFilter(PartitionColumns.of(cellname.column),
                                                  FBUtilities.<Clustering>singleton(cellname.clustering, metadata.comparator),
                                                  false);
            }
    ...
    throws RequestValidationException, UnavailableException, TimedOutException
    {
        ...
        try
        {
            ...
            DecoratedKey dk = StorageService.getPartitioner().decorateKey(key);
            PartitionUpdate update = new PartitionUpdate(metadata, dk, PartitionColumns.of(name.column), 1, FBUtilities.nowInSeconds());
            ...
            writer.writeCell(name.column, false, column.value, SimpleLivenessInfo.forUpdate(column.timestamp, column.ttl, update.nowInSec(), metadata), path);
            writer.endOfRow();
    ...
    throws RequestValidationException, UnavailableException, TimedOutException
    {
        ...
        {
            update = new PartitionUpdate(metadata, dk, PartitionColumns.NONE, 1, nowInSec);
