    ...
    public static class ParsedInsert extends ModificationStatement.Parsed
    {
                                                        ...
                                                        Attributes attrs)
        {
            ...
            for (int i = 0; i < columnNames.size(); i++)
            {
                ...
                {
                    Operation operation = new Operation.SetValue(value).prepare(keyspace(), def, cfm);
    ...
    public static class ParsedInsertJson extends ModificationStatement.Parsed
    {
                                                        ...
                                                        Attributes attrs)
        {
            ...
            for (ColumnDefinition def : defs)
            {
                ...
                {
                    Operation operation = new Operation.SetValue(raw).prepare(keyspace(), def, cfm);
                    operation.collectMarkerSpecification(boundNames);
                    operations.add(operation);
                }
            }
    ...
    public static class ParsedUpdate extends ModificationStatement.Parsed
    {
                                                        ...
                                                        Attributes attrs)
        {
            ...
            for (Pair<ColumnIdentifier.Raw, Operation.RawUpdate> entry : updates)
            {
                ...
                Operation operation = entry.right.prepare(keyspace(), def, cfm);
                operation.collectMarkerSpecification(boundNames);
                operations.add(operation);
            }
