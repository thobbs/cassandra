...
public class StorageProxy implements StorageProxyMBean
{
    ...
    throws UnavailableException, ReadFailureException, ReadTimeoutException
    {
        ...
        try
        {
            ...
            if (group.commands.size() > 1)
                result = group.limits().filter(result);
        }
        catch (UnavailableException e)
        {
            readMetrics.unavailables.mark();
        ...
        protected void runMayThrow()
        {
            try
            {
                ...
                try
                {
                    ...
                    handler.response(command.makeResponse(result, !handler.resolver.mayDigestData()));
                }
                catch (Throwable t)
                {
                    result.close();
    ...
    throws UnavailableException, ReadFailureException, ReadTimeoutException
    {
        ...
        return command.limits().filter(new RangeCommandIterator(ranges, command, concurrencyFactor, keyspace, consistencyLevel));
    }
