...
public class KeyspaceTest extends CQLTester
{
    ...
    private static void assertRowsInSlice(ColumnFamilyStore cfs, String key, int sliceStart, int sliceEnd, int limit, boolean reversed, String columnValuePrefix)
    {
        ...
        Slices slices = Slices.with(cfs.getComparator(), Slice.make(cfs.getComparator(), startClustering, endClustering));
    ...
    private static SlicePartitionFilter slices(ColumnFamilyStore cfs, Integer sliceStart, Integer sliceEnd, boolean reversed)
    {
        PartitionColumns columns = PartitionColumns.of(cfs.metadata.getColumnDefinition(new ColumnIdentifier("c", false)));
    ...
    private static SinglePartitionSliceCommand singlePartitionSlice(ColumnFamilyStore cfs, String key, SlicePartitionFilter filter, Integer rowLimit)
    {
        DataLimits limit = rowLimit == null
                         ? DataLimits.NONE
                         : DataLimits.cqlLimits(rowLimit);
        return new SinglePartitionSliceCommand(
                cfs.metadata, FBUtilities.nowInSeconds(), ColumnFilter.NONE, limit, Util.dk(key), filter);
    }
    ...
    public void testGetSliceFromBasic() throws Throwable
    {
        ...
        for (int round = 0; round < 2; round++)
        {
            ...
            SlicePartitionFilter filter = slices(cfs, 5, null, false);
    ...
    {
        SlicePartitionFilter filter = slices(cfs, 1000, null, false);
        SinglePartitionSliceCommand command = new SinglePartitionSliceCommand(
                cfs.metadata, FBUtilities.nowInSeconds(), ColumnFilter.NONE, DataLimits.cqlLimits(3), Util.dk("0"), filter);
        ...
        command = new SinglePartitionSliceCommand(
                cfs.metadata, FBUtilities.nowInSeconds(), ColumnFilter.NONE, DataLimits.cqlLimits(1000), Util.dk("0"), filter);
