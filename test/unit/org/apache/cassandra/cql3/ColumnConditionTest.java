...
public class ColumnConditionTest
{
    ...
    private static ColumnCondition.Bound in(ColumnDefinition definition, ByteBuffer... values) throws InvalidRequestException
    {
        List<Term> terms = new ArrayList<>(values.length);
        for (ByteBuffer value : values)
            terms.add(new Constants.Value(value));
        return ColumnCondition.inCondition(definition, terms).bind(QueryOptions.DEFAULT);
    }

    @Test
    public void testSimpleBoundEquality() throws InvalidRequestException
    {
        ColumnDefinition definition1 = new ColumnDefinition("ks", "cf", new ColumnIdentifier("col1", false), Int32Type.instance, null, null, null, null, null);
        ColumnDefinition definition2 = new ColumnDefinition("ks", "cf", new ColumnIdentifier("col2", false), Int32Type.instance, null, null, null, null, null);
        ColumnCondition condition1 = ColumnCondition.condition(definition1, new Constants.Value(ONE), Relation.Type.EQ);
        ColumnCondition.Bound bound1 = condition1.bind(QueryOptions.DEFAULT);

        assertTrue(bound1.equals(bound1));

        // different column names
        ColumnCondition condition2 = ColumnCondition.condition(definition2, new Constants.Value(ONE), Relation.Type.EQ);
        assertFalse(bound1.equals(condition2.bind(QueryOptions.DEFAULT)));

        // different operators
        condition2 = ColumnCondition.condition(definition1, new Constants.Value(ONE), Relation.Type.NEQ);
        assertFalse(bound1.equals(condition2.bind(QueryOptions.DEFAULT)));

        // both have null values
        condition1 = ColumnCondition.condition(definition1, Constants.NULL_VALUE, Relation.Type.EQ);
        condition2 = ColumnCondition.condition(definition1, Constants.NULL_VALUE, Relation.Type.EQ);
        assertTrue(condition1.bind(QueryOptions.DEFAULT).equals(condition2.bind(QueryOptions.DEFAULT)));

        // the first has a null value, the second does not
        condition2 = ColumnCondition.condition(definition1, new Constants.Value(ONE), Relation.Type.NEQ);
        assertFalse(condition1.bind(QueryOptions.DEFAULT).equals(condition2.bind(QueryOptions.DEFAULT)));

        // the first has a non-null value, the second has a null value
        condition1 = ColumnCondition.condition(definition1, new Constants.Value(ONE), Relation.Type.NEQ);
        condition2 = ColumnCondition.condition(definition1, Constants.NULL_VALUE, Relation.Type.EQ);
        assertFalse(condition1.bind(QueryOptions.DEFAULT).equals(condition2.bind(QueryOptions.DEFAULT)));

        // IN conditions
        assertTrue(in(definition1, ONE).equals(in(definition1, ONE)));
        assertTrue(in(definition1).equals(in(definition1)));

        // different values
        assertFalse(in(definition1, ONE).equals(in(definition1, TWO)));

        // different lengths
        assertFalse(in(definition1, ONE).equals(in(definition1, ONE, TWO)));
        assertFalse(in(definition1, ONE, TWO).equals(in(definition1, ONE)));
        assertFalse(in(definition1, ONE, TWO).equals(in(definition1)));

        // different orderings
        assertTrue(in(definition1, ONE, TWO).equals(in(definition1, TWO, ONE)));

        // nulls in the INs
        assertFalse(in(definition1, ONE).equals(in(definition1, new ByteBuffer[]{null})));
        assertFalse(in(definition1, new ByteBuffer[]{null}).equals(in(definition1, ONE)));
        assertTrue(in(definition1, ONE, null).equals(in(definition1, null, ONE)));

        // duplicate values
        assertTrue(in(definition1, ONE, ONE).equals(in(definition1, ONE)));
    }

    private static ColumnCondition.Bound elementIn(ColumnDefinition definition, ByteBuffer... values) throws InvalidRequestException
    {
        List<Term> terms = new ArrayList<>(values.length);
        for (ByteBuffer value : values)
            terms.add(new Constants.Value(value));
        return ColumnCondition.inCondition(definition, new Constants.Value(ONE), terms).bind(QueryOptions.DEFAULT);
    }
    ...
    public void testElementAccessBoundEquality() throws InvalidRequestException
    {
        List<ColumnDefinition> definitions = Arrays.asList(
                new ColumnDefinition("ks", "cf", new ColumnIdentifier("col1", false), ListType.getInstance(AsciiType.instance), null, null, null, null, null),
                new ColumnDefinition("ks", "cf", new ColumnIdentifier("col2", false), MapType.getInstance(Int32Type.instance, AsciiType.instance), null, null, null, null, null));

        ColumnCondition condition1 = ColumnCondition.condition(definitions.get(0), new Constants.Value(ONE), new Constants.Value(ONE), Relation.Type.EQ);
        ColumnCondition condition2 = ColumnCondition.condition(definitions.get(1), new Constants.Value(ONE), new Constants.Value(ONE), Relation.Type.EQ);
        // different column names
        assertFalse(condition1.bind(QueryOptions.DEFAULT).equals(condition2.bind(QueryOptions.DEFAULT)));

        for (ColumnDefinition definition : definitions)
        {
            ColumnCondition condition = ColumnCondition.condition(definition, new Constants.Value(ONE), new Constants.Value(A), Relation.Type.EQ);
            ColumnCondition.Bound bound = condition.bind(QueryOptions.DEFAULT);

            assertTrue(bound.equals(bound));

            // different operators
            condition2 = ColumnCondition.condition(definition, new Constants.Value(ONE), new Constants.Value(A), Relation.Type.NEQ);
            assertFalse(bound.equals(condition2.bind(QueryOptions.DEFAULT)));

            // different elements
            condition2 = ColumnCondition.condition(definition, new Constants.Value(TWO), new Constants.Value(A), Relation.Type.EQ);
            assertFalse(bound.equals(condition2.bind(QueryOptions.DEFAULT)));

            // both have null values
            condition = ColumnCondition.condition(definition, new Constants.Value(ONE), Constants.NULL_VALUE, Relation.Type.EQ);
            condition2 = ColumnCondition.condition(definition, new Constants.Value(ONE), Constants.NULL_VALUE, Relation.Type.EQ);
            assertTrue(condition.bind(QueryOptions.DEFAULT).equals(condition2.bind(QueryOptions.DEFAULT)));

            // the first has a null value, the second does not
            condition2 = ColumnCondition.condition(definition, new Constants.Value(ONE), new Constants.Value(A), Relation.Type.NEQ);
            assertFalse(condition.bind(QueryOptions.DEFAULT).equals(condition2.bind(QueryOptions.DEFAULT)));

            // the first has a non-null value, the second has a null value
            condition = ColumnCondition.condition(definition, new Constants.Value(ONE), new Constants.Value(A), Relation.Type.NEQ);
            condition2 = ColumnCondition.condition(definition, new Constants.Value(ONE), Constants.NULL_VALUE, Relation.Type.EQ);
            assertFalse(condition.bind(QueryOptions.DEFAULT).equals(condition2.bind(QueryOptions.DEFAULT)));

            // IN conditions
            assertTrue(elementIn(definition, A).equals(elementIn(definition, A)));
            assertTrue(elementIn(definition).equals(elementIn(definition)));

            // different values
            assertFalse(elementIn(definition, A).equals(elementIn(definition, B)));

            // different lengths
            assertFalse(elementIn(definition, A).equals(elementIn(definition, A, B)));
            assertFalse(elementIn(definition, A, B).equals(elementIn(definition, A)));
            assertFalse(elementIn(definition, A, B).equals(elementIn(definition)));

            // different orderings
            assertTrue(elementIn(definition, A, B).equals(elementIn(definition, B, A)));

            // nulls in the INs
            assertFalse(elementIn(definition, A).equals(elementIn(definition, new ByteBuffer[]{null})));
            assertFalse(elementIn(definition, new ByteBuffer[]{null}).equals(elementIn(definition, A)));
            assertTrue(elementIn(definition, A, null).equals(elementIn(definition, null, A)));

            // duplicate values
            assertTrue(elementIn(definition, A, A).equals(elementIn(definition, A)));
        }
    }

    private static ColumnCondition.Bound listCollectionIn(ColumnDefinition definition, List<ByteBuffer>... inValues) throws InvalidRequestException
    {
    ...
    private static ColumnCondition.Bound listCollectionWithNullIn(ColumnDefinition definition, List<ByteBuffer>... otherInValues) throws InvalidRequestException
    {
    ...
    {
        ColumnDefinition definition1 = new ColumnDefinition("ks", "cf", new ColumnIdentifier("col1", false), ListType.getInstance(AsciiType.instance), null, null, null, null, null);

        ColumnCondition condition1 = ColumnCondition.condition(definition1, new Lists.Value(list(ZERO, ONE, TWO)), Relation.Type.EQ);
        ColumnCondition.Bound bound1 = condition1.bind(QueryOptions.DEFAULT);

        assertTrue(bound1.equals(bound1));

        // different column names
        ColumnDefinition definition2 = new ColumnDefinition("ks", "cf", new ColumnIdentifier("col2", false), ListType.getInstance(AsciiType.instance), null, null, null, null, null);
        ColumnCondition condition2 = ColumnCondition.condition(definition2, new Lists.Value(list(ZERO, ONE, TWO)), Relation.Type.EQ);
        assertFalse(bound1.equals(condition2.bind(QueryOptions.DEFAULT)));

        // different operators
        condition2 = ColumnCondition.condition(definition1, new Lists.Value(list(ZERO, ONE, TWO)), Relation.Type.NEQ);
        assertFalse(bound1.equals(condition2.bind(QueryOptions.DEFAULT)));
        ...
        // the first has a non-null value, the second has a null value
        condition1 = ColumnCondition.condition(definition1, new Lists.Value(list(ZERO, ONE, TWO)), Relation.Type.NEQ);
        condition2 = ColumnCondition.condition(definition1, Constants.NULL_VALUE, Relation.Type.EQ);
        assertFalse(condition1.bind(QueryOptions.DEFAULT).equals(condition2.bind(QueryOptions.DEFAULT)));

        // IN conditions
        assertTrue(listCollectionIn(definition1, list(ONE)).equals(listCollectionIn(definition1, list(ONE))));
        assertTrue(listCollectionIn(definition1, list(ZERO), list(ONE, TWO)).equals(listCollectionIn(definition1, list(ZERO), list(ONE, TWO))));
        assertTrue(listCollectionIn(definition1).equals(listCollectionIn(definition1)));

        // different values
        assertFalse(listCollectionIn(definition1, list(ONE)).equals(listCollectionIn(definition1, list(TWO))));
        assertFalse(listCollectionIn(definition1, list(ONE, TWO)).equals(listCollectionIn(definition1, list(ONE))));

        // different IN lengths
        assertFalse(listCollectionIn(definition1, list(ONE)).equals(listCollectionIn(definition1, list(ONE), list(TWO))));
        assertFalse(listCollectionIn(definition1, list(ONE), list(TWO)).equals(listCollectionIn(definition1, list(ONE))));
        assertFalse(listCollectionIn(definition1, list(ONE), list(TWO)).equals(listCollectionIn(definition1)));

        // different orderings
        assertTrue(listCollectionIn(definition1, list(ONE), list(TWO)).equals(listCollectionIn(definition1, list(TWO), list(ONE))));

        // nulls in the INs
        assertFalse(listCollectionIn(definition1, list(ONE)).equals(listCollectionWithNullIn(definition1)));
        assertFalse(listCollectionWithNullIn(definition1).equals(listCollectionIn(definition1, list(ONE))));
        assertTrue(listCollectionWithNullIn(definition1, list(ONE)).equals(listCollectionWithNullIn(definition1, list(ONE))));

        // duplicate values
        assertTrue(listCollectionIn(definition1, list(ONE), list(ONE)).equals(listCollectionIn(definition1, list(ONE))));
    }
    ...
    private static ColumnCondition.Bound mapCollectionIn(CFDefinition.Name name, Map<ByteBuffer, ByteBuffer>... inValues) throws InvalidRequestException
    {
        ...
        return ColumnCondition.inCondition(name, terms).bind(QueryOptions.DEFAULT);
    }
    ...
    public void testMapCollectionBoundEquality() throws InvalidRequestException
    {
        ...
        // both have null values
        condition1 = ColumnCondition.condition(name1, Constants.NULL_VALUE, Relation.Type.EQ);
        condition2 = ColumnCondition.condition(name1, Constants.NULL_VALUE, Relation.Type.EQ);
        assertTrue(condition1.bind(QueryOptions.DEFAULT).equals(condition2.bind(QueryOptions.DEFAULT)));
