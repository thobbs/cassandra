...
Any columns which are ommitted from the @JSON@ map will be defaulted to a @NULL@ value (which may result in a tombstone being created).
...
|@bigint@   |integer, float, string|integer          |Floats are truncated; String must be valid integer; subject to overflow|
...
|@int@      |integer, float, string|integer          |Floats are truncated; String must be valid integer; subject to overflow|
...
|@varint@   |integer, float, string|integer          |Variable length; may overflow 32 or 64 bit integers|
